...
```{r}
model_rf <- randomForest(form = model_formula, data = training, mtry = 2, ntree = 100)
```
...
```{r}
#Make predictions by evaluating randomForests model on test dataset
test <- test %>% 
  mutate(
...
#Write csv file to sumbit to Kaggle and get RMSLE score for our model
write_csv(submission_rf, "data/submission_rf.csv")
```


![](images/forando_rf.png){ width=100% }
